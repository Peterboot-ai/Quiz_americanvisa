...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // super_admin has no tenant on auth — resolve from request
  const tenant = auth.tenant ?? await resolveTenant(req);

  if (!tenant) {
    return res.status(400).json({ success: false, error: 'Tenant não encontrado. Use ?tenant=slug na URL.' });
  }

  if (!tenant.brevo_api_key) {
    return res.status(400).json({ success: false, error: 'Brevo API key não configurada para este tenant' });
  }

  if (!tenant.report_recipients?.length) {
    return res.status(400).json({ success: false, error: 'Nenhum destinatário de relatório configurado para este tenant' });
  }
  ...
  try {
    ...
    const emailRes = await fetch('https://api.brevo.com/v3/smtp/email', {
      ...
      body: JSON.stringify({
        ...
        to: tenant.report_recipients.map((email: string) => ({ email })),
        subject: yesterdayCount === 0
          ? `[TESTE] ⚠️ Nenhum lead ontem — ${tenant.name}`
          : `[TESTE] 📊 ${yesterdayCount} lead${yesterdayCount !== 1 ? 's' : ''} ontem — ${tenant.name}`,
        htmlContent: html,
      }),
    });
    ...
    if (!emailRes.ok) {
      ...
      return res.status(500).json({ success: false, error: 'Falha ao enviar e-mail', details: detail });
    }
    ...
    return res.status(200).json({
      ...
      recipients: tenant.report_recipients,
    });
  } catch (err) {
    console.error('Error sending test report:', err);
    return res.status(500).json({ success: false, error: 'Falha ao enviar relatório de teste' });
  }
}
